...
def verify(avg_score, seed):
    ...
    cfg_dir = os.path.join(Path(args.path).parent, "best2_model_"+args.path.split("/")[-1].split("models_")[-1])
    ...
    for sdetec in sdetecs:
        ...
        for model in models:
            ...
            if test_result > best_result:
                best_result = copy.deepcopy(test_result)
                best_model_path = copy.deepcopy(mpath)
                best_model = lmodel
...
if __name__ == '__main__':
    ...
    for sdetec, scores in avg_score.items():
        avg_score[sdetec] = np.mean(scores)
